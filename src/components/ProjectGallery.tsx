...
function WorkstreamCard({ workstream, index }: { workstream: (typeof WORKSTREAMS)[number]; index: number }) {
  ...
  const updatePreviewPlacement = () => {
    ...
    const card = preview.parentElement?.getBoundingClientRect();
    if (!card) return;

    const previewHeight = preview.offsetHeight + 8;
    const spaceAbove = card.top;
  ...
  return (
    ...
    >
      ...
      <div
        ref={previewRef}
        className={`pointer-events-none absolute right-0 z-20 hidden w-56 scale-[0.97] overflow-hidden border border-border bg-card p-2 opacity-0 shadow-[var(--shadow-elevated)] transition duration-200 group-hover:scale-100 group-hover:opacity-100 md:block ${previewPositionClass}`}
      >
